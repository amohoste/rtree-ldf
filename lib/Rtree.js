            ...
            if (startHeight - current["rtree:height"] < maxHeight && current["rtree:height"] !== startCut) {
                ...
                if (current["rtree:height"] === 1) {
                    ...
                    current["rtree:children"].forEach((child_id, i) => {
                        let childnode = this.getNode(child_id)
                        let value = `POLYGON ((${childnode.maxX} ${childnode.minY}, ${childnode.maxX} ${childnode.maxY}, ${childnode.minX} ${childnode.maxY}, ${childnode.minX} ${childnode.minY}, ${childnode.maxX} ${childnode.minY}))`;
                        childRelation["tree:child"][i] = { "@id": path.join('/', treeDir, `t${treePage}.jsonld#${child_id}`), "@type": "tree:Node", "value": value};
