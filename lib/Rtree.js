...
class Rtree {
    ...
    toFragments(obj) {
        ...
        fs.writeFileSync(path.join(obj.outDir, obj.collection), JSON.stringify(createCollectionDescription(obj.collection, obj.manages, this.totalItems, path.join(obj.treeDir, 't0.jsonld'))));

        // Calculate optimal height
        let optheight = this.getOptimalHeight(goalSize, maxSize);

        // Convert tree to fragments
        this._toFragments(this.root.convertFrag(`t${startTreePage}`), this.root.height, optheight - 1, this.getStartCut(this.root.height, optheight), [startTreePage, startDataPage], goalSize, obj.outDir, obj.dataDir, obj.treeDir);
    }
        ...
        while(stack.length > 0) {
            ...
            if (startHeight - current["rtree:height"] < maxHeight && current["rtree:height"] !== startCut) {
                ...
                if (current["rtree:height"] !== 1) {
                    ...
                    childRelation["tree:child"] = current["rtree:children"].map((child_id) => this.getNode(child_id).convertFrag(`t${treePage}`));
                    ...
                    dataFrag["@graph"] = dataFrag["@graph"].concat(current["rtree:children"].map((child_id) => {
                        let frag = this.getNode(child_id).convertFrag(`d${dataPage}`);
                    ...
                    current["hydra:member"] = current["rtree:children"].map((child_id) => {
                        let obj =  { "@id": path.join('/', dataDir, `d${dataPage}#${child_id}`) };
                ...
                if (current["rtree:height"] === 1) {
                    ...
                    current["rtree:children"].forEach((child_id, i) => {
                        childRelation["tree:child"][i] = { "@id": path.join('/', dataDir, `t${treePage}#${child_id}`), "@type": "tree:Node" };
                        let node = this.getNode(child_id).convertFrag(`t${treePage}`);
    ...
    getSubFragtree(node) {
        ...
        while(stack.length > 0) {
            ...
            current["rtree:children"] = current["rtree:children"].map((child_id) => this.getNode(child_id).convertFrag());

            for (let i = 0; i < current["rtree:children"].length; i++) {
                if (current["rtree:height"] !== 1) {
                    stack.push(current["rtree:children"][i]);
                }
            }
