...
class Rtree {
    ...
    toFragments(obj) {
        ...
        fs.writeFileSync(path.join(obj.outDir, obj.collection), JSON.stringify(createCollectionDescription(obj.collection, obj.manages, this.totalItems, path.join(obj.treeDir, `t${startTreePage}#${this.root.id}.jsonld`))));
            ...
            if (startHeight - current["rtree:height"] < maxHeight && current["rtree:height"] !== startCut) {
                ...
                if (current["rtree:height"] === 1) {
                    ...
                    current["rtree:children"].forEach((child_id, i) => {
                        childRelation["tree:child"][i] = { "@id": path.join('/', dataDir, `t${treePage}.jsonld#${child_id}`), "@type": "tree:Node" };
